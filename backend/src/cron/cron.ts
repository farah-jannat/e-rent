...
import { db } from "@/db";
import { flats, rentPayment, tenants } from "@/schemas";
...
export const initCronJobs = () => {
  ...
  cron.schedule("0 0 1 * *", async () => {
    ...
    const activeTenants = await db
      .select()
      .from(tenants)
      .where(eq(tenants.isArchived, false));
    ...
    for (const tenant of activeTenants) {
      ...
      const flatData = await db
        .select()
        .from(flats)
        .where(eq(flats.id, tenant.flatId!))
        .limit(1);
    ...
    console.log(
      `Successfully generated bills for ${activeTenants.length} tenants.`,
    );
  });
};
